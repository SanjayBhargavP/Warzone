					...
					switch (l_commands[0]) {
						...
						case "loadmap":
							...
							if (l_commands.length == 2) {
								...
								System.out.println(
										ColorCoding.red + "No map entered. Kindly enter exact name of map to be loaded"
												+ ColorCoding.blank);
							}
						...
						case "gameplayer":
							...
							for (int i = 1; i < l_commands.length;) {
								...
								if (l_commands[i].equals("-add")) {
									...
									System.out.println(ColorCoding.green + l_player.getD_playerName()
											+ " added successfully" + ColorCoding.blank);
									i = i + 2;
									...
									for (int j = 0; j < l_playersArray.size(); j++) {
										if (l_commands[i + 1].equals(l_playersArray.get(j).getD_playerName())) {
											System.out
													.println(ColorCoding.green + l_playersArray.get(j).getD_playerName()
															+ " removed successfully" + ColorCoding.blank);
						...
						case "assigncountries":
							...
							if (l_playersArray.size() > 0) {
								if (PlayersGameplay.assigncountries(l_playersArray,
									...
									System.out.println(
											ColorCoding.green + "Countries assigned to the players successfully"
													+ ColorCoding.blank + "\n");
								...
								System.out.println(
										ColorCoding.red + "ERROR: No players to assign Countries" + ColorCoding.blank);
							}
			...
			if (l_gamePhase.equals("mainGameLoop")) {
				...
				for (int i = 0; i < l_playersArray.size(); i++) {
					System.out.println("Player " + Integer.sum(i, 1) + "(" + l_playersArray.get(i).getD_playerName()
							+ ") has Army Count: " + l_playersArray.get(i).getD_armyCount());
					...
					for (int i = 0; i < l_playersArray.size(); i++) {
						...
						if (l_playersArray.get(i).getD_armyCount() != 0) {
							...
							if (PlayersGameplay.checkArmyAvailable(Integer.parseInt(l_tempOrderListArray[2]),
								...
								System.out.println(ColorCoding.red + "Error: Please enter valid number of troops"
										+ ColorCoding.blank);
								i--;
							}
				...
				System.out.println(ColorCoding.green
						+ "All Armies have been successfully deployed. Enter command to proceed " + ColorCoding.blank);
			}
