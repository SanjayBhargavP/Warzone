 ...
 * 
 * 
 *         The GameEngine class serves as the entry point for the Warzone game,
 *         managing player commands and game phases.
 *
 */
		...
		do {
			if (l_flag == 0) {
				...
				if (l_commands[0] != null) {
					switch (l_commands[0]) {
						...
						case "editcontinent":
							...
							for (int i = 1; i < l_commands.length;) {
								...
								if (l_commands[i].equals("-add")) {
									...
									MapEditor.addContinent(l_commands[i + 1], Integer.parseInt(l_commands[i + 2]),
											l_mainConnectivity);
									i = i + 3;
								}
						...
						case "editcountry":
							...
							for (int i = 1; i < l_commands.length;) {
								if (l_commands[i].equals("-add")) {
									...
									System.out.println(
											ColorCoding.ANSI_RED + "ERROR: Invalid Command" + ColorCoding.ANSI_RESET);
								}
							}
						...
						case "editneighbor":
							...
							for (int i = 1; i < l_commands.length;) {
								if (l_commands[i].equals("-add")) {
									...
									MapEditor.addNeighbour(Integer.parseInt(l_commands[i + 1]),
											Integer.parseInt(l_commands[i + 2]), l_mainConnectivity);
									i = i + 3;
								}

								else if (l_commands[i].equals("-remove")) {
									System.out.println("Removing neighbour...");
									MapEditor.removeNeighbour(Integer.parseInt(l_commands[i + 1]),
											Integer.parseInt(l_commands[i + 2]), l_mainConnectivity, 1);
									i = i + 3;
								}
						...
						case "showmap":
							l_gamePhase = "startup";
							ShowMap.showMap(l_mainConnectivity.getD_continentList(),
									l_mainConnectivity.getD_countryList());
							break;
						...
						case "loadmap":
							...
							if (l_commands.length == 2) {
								...
								System.out.println(ColorCoding.ANSI_RED
										+ "No map entered. Kindly enter exact name of map to be loaded"
						...
						case "gameplayer":
							...
							for (int i = 1; i < l_commands.length;) {
								...
								if (l_commands[i].equals("-add")) {
									...
									System.out.println(ColorCoding.ANSI_GREEN + l_player.getD_playerName()
											+ " added successfully" + ColorCoding.ANSI_RESET);
									i = i + 2;
									...
									for (int j = 0; j < l_playersArray.size(); j++) {
										if (l_commands[i + 1].equals(l_playersArray.get(j).getD_playerName())) {
											System.out.println(
													ColorCoding.ANSI_GREEN + l_playersArray.get(j).getD_playerName()
															+ " removed successfully" + ColorCoding.ANSI_RESET);
						...
						case "assigncountries":
							...
							if (l_playersArray.size() > 0) {
								if (PlayersGameplay.assignCountries(l_playersArray,
										l_mainConnectivity.getD_countryList(),
										l_mainConnectivity.getD_continentList()) == 0) {
									System.out.println(
											ColorCoding.ANSI_GREEN + "Countries assigned to the players successfully"
													+ ColorCoding.ANSI_RESET + "\n");
								...
								System.out.println(ColorCoding.ANSI_RED + "ERROR: No players to assign Countries"
										+ ColorCoding.ANSI_RESET);
							}
			...
			if (l_gamePhase.equals("mainGameLoop")) {
				...
				for (int i = 0; i < l_playersArray.size(); i++) {
					System.out.println("Player " + Integer.sum(i, 1) + "(" + l_playersArray.get(i).getD_playerName()
							+ ") has Army Count: " + l_playersArray.get(i).getD_armyNumber());
				...
				while (l_temp > 0) {
					...
					for (int i = 0; i < l_playersArray.size(); i++) {
						...
						if (l_playersArray.get(i).getD_armyNumber() != 0) {
							System.out.println(
									"Player " + l_playersArray.get(i).getD_playerName() + " deploy your troops:");
							...
							for (int j = 0; j < l_playersArray.get(i).getD_country().size(); j++) {
								if (Integer.parseInt(l_tempOrderListArray[1]) == (l_playersArray.get(i).getD_country()
										.get(j).getD_countryId())) {
									l_order.setD_sourceCountry(l_playersArray.get(i).getD_country().get(j));
								}
							}

							if (PlayersGameplay.checkArmyAvailable(Integer.parseInt(l_tempOrderListArray[2]),
									l_playersArray.get(i))) {
								...
								System.out.println(ColorCoding.ANSI_RED + "Error: Please enter valid number of troops"
										+ ColorCoding.ANSI_RESET);
								i--;
							}
				...
				while (l_temp > 0) {
					for (int i = 0; i < l_playersArray.size(); i++) {
						if (l_playersArray.get(i).getD_armyNumber() != 0) {
							l_playersArray.get(i).getD_order().execute(l_playersArray.get(i),
									l_playersArray.get(i).next_order());
							if (l_playersArray.get(i).getD_armyNumber() == 0) {
								flag += 1;
							}
						}
				...
				System.out.println(
						ColorCoding.ANSI_GREEN + "All Armies have been successfully deployed. Enter command to proceed "
								+ ColorCoding.ANSI_RESET);
			}
